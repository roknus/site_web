<?php
	...
	function print_comments($post_id){
		$db = connect_db();
		$request = $db->query('SELECT *,comments.id AS commentID FROM comments,membre,pictures  WHERE comments.id_poster = membre.id AND membre.image_profil = pictures.id AND id_post='.$post_id.' ORDER BY creation_time ASC;');
		$mess = "";
		while($data = $request->fetch()){
			$mess .='
				<tr class="comments">
					...
					-
					<button class="button_like" onclick="like('.$data["commentID"].',this)">'.get_likes($data["commentID"]).'</button>
					</td>
				</tr>';
		}
		return $mess;
